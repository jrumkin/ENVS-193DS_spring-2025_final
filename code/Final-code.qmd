...
plot(residuals1, main = "Null")
plot(residuals2, main = "Saturated")
plot(residuals3, main = "Model 3")
plot(residuals4, main = "Model 4")
...
model_df <- as.data.frame(model_prediction) #make the object into a data frame to use in ggplot properly

ggplot(data = model_df,
       aes( x = x,
            y = predicted,
            color = group,
            fill = group)) + #probability occupied by a swift parrot
  geom_point() +
  geom_ribbon(
              aes(ymin = conf.low,
                  ymax = conf.high),
  ...
  scale_fill_manual(values = c("2016" = "#9544BF",
                                "2019" = "#4465BF")) +
  scale_color_manual(values = c("2016" = "#9544BF",
                                "2019" = "#4465BF"))
...
**Fig. 1 Swift Parrots have a lower probability of occupancy with greater distance to forest edge.** Data from DRYAD "Do nest boxes breed the target species or its competitors? A case study of a critically endangered bird." The points represent individual predictions using the saturated generalized linear model with lines connecting them. The ribbons represent the confidence interval of the predictions. The color represent data from each season with 2016 in purple and 2019 in blue.
