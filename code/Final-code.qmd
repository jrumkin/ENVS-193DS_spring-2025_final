...
mod1 <-  glm(sp ~ 1, #null model #running a generalized linear model because the data is continuous against binary
                   data = nest_clean,
...
plot(residuals1, main = "Model 1, Null")
plot(residuals2, main = "Model 2, Saturated")
plot(residuals3, main = "Model 3, year only")
plot(residuals4, main = "Model 4, distance only")
...
Akaike’s Information Criterion defined model 2 as the best model. This model says that Swift Parrot presence in the nest boxes is best predicted by considering the season and the distance to forest edge.
...
model_plot <-  plot(model_prediction, show_data = TRUE) #use the ggeffects and base plot() function to make a plot with the prediction ribbon
...
ggplot(model_df, #using ggplot to add customizations and geometry layers to the plot
  ...
  geom_point(size = 1.2) +  # adds the prediction points
  geom_jitter(data = nest_explore,  #underlying data points (binary occupancy) (minimal jitter the points for legibility and reduce overlap)
              ...
              inherit.aes = FALSE, #doesn't inherit aesthics from orignal call on ggplot
              height = 0.02, #minimal height jitter to increase legibility
              alpha = 0.35, #makes the data  35% transparent
              size = 1, #manual size of the data 
              shape = 2) + #gives the data an open triangle shape to seperate them from the prediction data points
  scale_color_manual(values = c("2016" = "#9544BF", "2019" = "#4465BF")) + #maual color for points and lines
...
Probability of a Swift Parrot occupying a nest box is highest when the nest box is close to the forest edge both for when the nest boxes are first installed and three years after installation which can be seen in Figure 1. Swift Parrots are also more likely to occupy a nest box in the first season the boxes are installed, which can be seen in the calculated model predictions. Swift Parrots rarely return to a previous nesting site, so their use of nest boxes declines over time. The other bird species present, Tree Martin and Common Starling, also both present dominant behavior over the Swift Parrot and will occupy the nests when the Swift Parrots are not present.
